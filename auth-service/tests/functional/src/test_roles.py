...
@pytest.mark.asyncio
async def test_read_roles(make_get_http_request, create_role, cleanup_role, role_data, role_data2, role_data3):
...
@pytest.mark.asyncio
async def test_update_role(make_put_http_request, create_role, cleanup_role, role_data, role_data2, login_as_admin):
    role = await create_role(role_data)
    role_id = role["id"]
    access_token = login_as_admin
    headers = {
...
@pytest.mark.asyncio
async def test_delete_role(make_delete_http_request, create_role, cleanup_role, role_data, login_as_admin):
    role = await create_role(role_data)
    role_id = role["id"]
    access_token = login_as_admin
    headers = {
    ...
    status, body = await make_delete_http_request(endpoint=f'roles/{role_data["name"]}', headers=headers)
    assert status == HTTPStatus.OK, "Expected HTTP 200 OK"
...
@pytest.mark.asyncio
async def test_read_role_404(make_get_http_request, create_role, cleanup_role, role_data):
    role = await create_role(role_data)
    role_id = role["id"]
