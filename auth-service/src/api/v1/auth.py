...
async def register(user: UserRegistration,
    ...
    try:
        ...
        return ResponseMessage(message="Registration successful. Please log in using /login.")
    except AuthException as e:
...
async def authorize_provider(provider: str,
    ...
    try:
        user: UserProvider = await social_service.authorize(provider_name=provider, request=request)
...
async def login_provider(provider: str,
    ...
    try:
        return RedirectResponse(url=provider_service.get_authorization_url(provider_name=provider))
    except SocialException as e:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=e.message)
...
async def refresh_tokens(refresh_token: str = Header(None),
                         user_agent: Annotated[str | None, Header()] = None,
                         auth_service: IAsyncAuthService = Depends(get_auth_service)) -> ResponseAuthTokens:
    ...
    try:
        return await auth_service.refresh_tokens(refresh_token=refresh_token, user_agent=user_agent)
    except AuthException as e:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,
                            detail=e.message,
                            headers={"WWW-Authenticate": "Bearer"})
