...
class AuthService(IAsyncAuthService):
    ...
    async def add_db_user_session(self, user_id: UUID, refresh_token: str, user_agent: str) -> None:
        await self._db_session.add_user_session(user_id=user_id, refresh_token=refresh_token, user_agent=user_agent)

    async def password_change(self,
                              user: UserPassUpdate,
                              **kwargs) -> None:

        user_db = await self._db_user.authenticate_user(login=user.login, password=user.old_password)
        if user_db is None:
            raise AuthException(message="The login or password is incorrect.")

        await self._db_user.update_user_by_login(login=user_db.login, password=user.new_password)
    ...
    async def register(self,
        ...
        if role is None:
            ...
            raise AuthException(message="The role does not exist! The database is not initialized correctly!")
    ...
    async def login(self,
                    ...
                    user_agent: str,
                    **kwargs) -> ResponseAuthTokens:
        logger.info(f"Login user:{user.login}")

        user_db = await self._db_user.authenticate_user(login=user.login,
    ...
    async def _login_existing_user(self, user: User, user_agent: str) -> ResponseAuthTokens:
        roles: Sequence[Role] = await self._db_user.get_user_roles_by_id(user_id=user.id)

        _access_token, _refresh_token = await self._jwt.create_tokens(subject=user.login,
                                                                      role={"role": [r.name for r in roles]})
        await self._db_session.add_user_session(user_id=user.id,
                                                refresh_token=_refresh_token,
                                                user_agent=user_agent)
        return ResponseAuthTokens(access_token=_access_token, refresh_token=_refresh_token)

    async def _authorize_provider_new_user(self, user: UserProvider, user_agent: str, **kwargs):

        user_db = await self.register(user=UserRegistration(email=user.login, password=user.password),
                                      user_agent=user_agent)
        logger.info(f"Register new user:{user.login} with provider:{user.provider.value}")
        await self._db_provider.add_provider_account(provider_account=ProviderAccount(user_id=user_db.id,
                                                                                      id_social=user.id_social,
                                                                                      provider_name=user.provider))

        return await self.login(UserCredentials(email=user.login, password=user.password), user_agent=user_agent)
    ...
    async def authorize_provider(self, user: UserProvider, user_agent: str, **kwargs) -> ResponseAuthTokens:
        ...
        if user_db is None:
            return await self._authorize_provider_new_user(user=user, user_agent=user_agent)

        provider_account = await self._db_provider.get_provider_account_by_login(login=user.login,
                                                                                 provider_name=user.provider)
    ...
    async def refresh_tokens(self, refresh_token: str, user_agent: str, **kwargs) -> ResponseAuthTokens:
        user_db = await self._db_user.get_user_by_refresh_token(refresh_token=refresh_token)
        if user_db is None:
        ...
        roles: Sequence[Role] = await self._db_user.get_user_roles_by_id(user_id=user_db.id)
        role = {"role": [r.name for r in roles]}

        _access_token, _refresh_token = await self.create_tokens(user=user_db, role=role)
        await self.add_db_user_session(user_id=user_db.id,
                                       refresh_token=_refresh_token,
                                       user_agent=user_agent)
    ...
    async def logout(self, access_token: str) -> None:
        ...
        if await self.is_token_revoked(access_token=access_token):
            raise AuthException(message="Your current session has been terminated. Please log in again to continue.")
    ...
    async def logout_from_all_devices(self, access_token: str) -> None:
        ...
        if user is None:
            raise AuthException(message="Invalid or expired refresh token or no active sessions")
