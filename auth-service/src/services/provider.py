...
class ProviderService(IProviderService):
    ...
    async def authorize(self, provider_name: str, request: Request) -> UserProvider:
        ...
        try:
            access_token = await provider.get_access_token(query_params=dict(request.query_params))
        ...
        try:
            return UserProvider.from_provider(data=user_info, provider_name=provider_name)
