...
from async_fastapi_jwt_auth.exceptions import (InvalidHeaderError,
                                               JWTDecodeError)
...
class JWTManager(IAsyncJWTManager):
    ...
    async def create_access_token(self, subject: str, role: dict, **kwargs) -> str:
        return await self._auth_jwt.create_access_token(subject=subject,
                                                        user_claims=role)

    async def create_refresh_token(self, subject: str, role: dict, **kwargs) -> str:
        return await self._auth_jwt.create_refresh_token(subject=subject,
                                                         user_claims=role)

    async def create_tokens(self, subject: str, role: dict, **kwargs) -> tuple[str, str]:
        return (
    ...
    async def refresh_tokens(self, refresh_token: str, role: dict[str, list[str]], **kwargs) -> tuple[str, str]:
